...
// ==========================================
// EXTRACTION DES OPTIONS DU CATALOGUE (JSON)
// ==========================================
...
// =========================================================================
// 🔄 SIMULATION DU PORTEFEUILLE (Idéal pour tes tests)
// =========================================================================
$mode_test = true; // Passe à false quand ta table portefeuilles ou solde sera active en BDD

if ($mode_test) {
    $solde_actuel = 15000; // Simule un solde suffisant. Mets 500 pour tester le blocage.
} else {
    // Ton code d'origine si le portefeuille est actif en BDD :
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['confirmer_rdv'])) {
    ...
    // --- ZONE AUTOMATIQUE DE NETTOYAGE DES CONTRAINTES DE BASE ---
    try {
        ...
        // Passe si déjà nettoyé
    }

    // CALCUL DU COÛT REEL DU RDV (BASE + OPTIONS COCHÉES)
    $surcout_options = 0;
    ...
    if ($solde_actuel < $prix_total_calcule) {
        $message = "<div class='alert alert-danger text-center fw-bold'>❌ Solde insuffisant pour geler les fonds de ce RDV (" . number_format($prix_total_calcule, 0, ',', ' ') . " FCFA requis). Votre solde est de " . number_format($solde_actuel, 0, ',', ' ') . " FCFA.</div>";
    } else {

        // TA REQUÊTE D'INSERTION D'ORIGINE
        ...
        if ($insert->execute([$id_client, $id_coiffeur, $id_prestation, $date_rdv, $heure_debut, $heure_fin])) {
            ...
            // Si on n'est pas en mode test, on peut soustraire le solde en BDD
            if (!$mode_test) {
                $nouveau_solde = $solde_actuel - $prix_total_calcule;
                $update_solde = $pdo->prepare("UPDATE users SET solde = ? WHERE id = ?");
                $update_solde->execute([$nouveau_solde, $id_client]);
            }
            
            $solde_actuel -= $prix_total_calcule; // Mise à jour visuelle du solde pour l'affichage

            $message = "
                <div class='alert alert-success text-start p-3 fw-bold'>
                    🎉 Demande de rendez-vous envoyée au coiffeur !<br>
                    <span class='small text-dark d-block mt-2'>💰 " . number_format($prix_total_calcule, 0, ',', ' ') . " FCFA ont été gelés sur votre compte. Solde restant : " . number_format($solde_actuel, 0, ',', ' ') . " FCFA " . ($mode_test ? '(Simulé)' : '') . "</span>
                </div>";
        } else {
            ...
            <div class="col-md-6">
                ...
                <div class="card p-4 shadow-lg" style="background-color: #111; border: 1px solid var(--gold); border-radius: 15px;">
                    ...
                    <div class="p-3 mb-4 rounded bg-dark border border-secondary">
                        ...
                        <div class="mt-1 text-center text-muted small">💼 Mon solde disponible : <span class="text-warning fw-bold"><?php echo number_format($solde_actuel, 0, ',', ' '); ?> FCFA</span></div>
...
// 🔄 RÉCUPÉRATION DES PARAMÈTRES DE L'URL (Corrigé pour matcher le profil et l'agenda)
$id_prestation = isset($_GET['presta_id']) ? intval($_GET['presta_id']) : (isset($_GET['id_prestation']) ? intval($_GET['id_prestation']) : 0);
$id_coiffeur = isset($_GET['coiffeur_id']) ? intval($_GET['coiffeur_id']) : 0;

// ➕ Récupération de la date et de l'heure envoyées par l'agenda interactif
$date_pre_remplie = isset($_GET['date_rdv']) ? htmlspecialchars($_GET['date_rdv']) : '';
$heure_pre_remplie = isset($_GET['heure_debut']) ? htmlspecialchars($_GET['heure_debut']) : '';
        ...
        if ($insert->execute([$id_client, $id_coiffeur, $id_prestation, $date_rdv, $heure_debut, $heure_fin])) {
            ...
            $message = "
                <div class='alert alert-success text-start p-3 fw-bold'>
                    🎉 Demande de rendez-vous envoyée au coiffeur !<br>
                    <span class='small text-dark d-block mt-2'>💰 " . number_format($prix_total_calcule, 0, ',', ' ') . " FCFA ont été gelés sur votre compte. Solde restant : " . number_format($solde_actuel, 0, ',', ' ') . " FCFA " . ($mode_test ? '(Simulé)' : '') . "</span>
                    <span class='text-muted small d-block mt-1'>🔄 Redirection vers vos rendez-vous...</span>
                </div>
                <script>
                    setTimeout(function(){
                        window.location.href = '/coiffons/client/mes_rendezvous.php';
                    }, 3000);
                </script>";
        } else {
            $message = "<div class='alert alert-danger text-center fw-bold'>Une erreur est survenue, veuillez réessayer.</div>";
            ...
            <div class="col-md-6">
                ...
                <div class="card p-4 shadow-lg" style="background-color: #111; border: 1px solid var(--gold); border-radius: 15px;">
                    ...
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            <label class="text-white small mb-1">Date du rendez-vous</label>
                            <input type="date" name="date_rdv" class="form-control" min="<?php echo date('Y-m-d'); ?>" value="<?php echo $date_pre_remplie; ?>" required>
                        </div>

                        <div class="row">
                            <div class="col-6 mb-3">
                                <label class="text-white small mb-1">Heure de début</label>
                                <input type="time" id="heure_debut" name="heure_debut" class="form-control" value="<?php echo $heure_pre_remplie; ?>" required>
                            </div>
                            <div class="col-6 mb-3">
                                <label class="text-white small mb-1">Heure de fin </label>
                                <input type="time" id="heure_fin" name="heure_fin" class="form-control bg-dark border-secondary text-warning fw-bold" readonly required>
...
// ➕ Exécute le calcul au chargement au cas où les données sont pré-remplies par l'agenda
window.addEventListener('DOMContentLoaded', actualiserDureeEtPrix);
