...
if ($role_actuel === 'client') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT p.*, u.nom as nom_coiffeur, u.quartier FROM prestations p JOIN users u ON p.id_coiffeur = u.id_user WHERE u.role = 'coiffeur' AND u.abonnement_status = 1 AND u.ville = ? ORDER BY RAND() LIMIT 6");
        $stmt->execute([$ville_client]);
        $coiffeurs_matching = $stmt->fetchAll();
        
        // Si aucune coiffure dans sa ville, on prend globalement en BD
        if (empty($coiffeurs_matching)) {
            $coiffeurs_matching = $pdo->query("SELECT p.*, u.nom as nom_coiffeur, u.quartier FROM prestations p JOIN users u ON p.id_coiffeur = u.id_user WHERE u.abonnement_status = 1 ORDER BY RAND() LIMIT 6")->fetchAll();
        }
    } catch (Exception $e) {
        $coiffeurs_matching = [];
    }

    // SI LA BD EST VIDE : On injecte les coiffures par défaut adaptées à la ville du client si possible
    if (empty($coiffeurs_matching)) {
        $coiffeurs_matching = $coiffures_par_defaut;
        // Petit bonus : on filtre notre tableau par défaut pour mettre sa ville en priorité
        usort($coiffeurs_matching, function($a, $b) use ($ville_client) {
            if ($a['ville'] === $ville_client && $b['ville'] !== $ville_client) return -1;
            if ($a['ville'] !== $ville_client && $b['ville'] === $ville_client) return 1;
            return 0;
        });
    }
    ...
    // SI LA BD EST VIDE : On injecte directement le catalogue par défaut
...
<?php elseif (isset($_SESSION['role']) && $_SESSION['role'] === 'client'): ?>
    ...
    <section class="py-5 bg-dark">
        <div class="container">
            <h3 class="text-white mb-4"><i class="bi bi-geo-alt-fill text-warning me-2"></i> Les styles disponibles à <?php echo htmlspecialchars($_SESSION['ville']); ?></h3>
            <div class="row g-4">
                <?php foreach ($coiffeurs_matching as $coif): ?>
                    <div class="col-md-4">
                        <div class="card bg-black border-secondary text-white h-100 shadow card-hover">
...
<?php else: ?>
    <section class="py-5 text-center bg-black border-bottom border-secondary" style="min-height: 45vh; display: flex; align-items: center;">
        <div class="container">
            <h1 class="text-warning fw-bold display-4 mb-3">COIFFONS — VOTRE SALON DE LUXE À DOMICILE</h1>
            <p class="text-light fs-5 mb-4 mx-auto" style="max-width: 750px;">
